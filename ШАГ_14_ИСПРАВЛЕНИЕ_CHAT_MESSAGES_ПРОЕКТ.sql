-- =====================================================
-- ШАГ 14: ИСПРАВЛЕНИЕ RLS ДЛЯ ТАБЛИЦЫ CHAT_MESSAGES (ПРОЕКТ)
-- =====================================================
-- Таблица: сообщения в чатах проекта
-- Логика: 1 проект = множество сообщений (командная работа)
...
DROP POLICY IF EXISTS "chat_messages_combined_access" ON public.chat_messages;
...
UNION ALL
SELECT 
    'Сообщений с user_id = null' as metric,
    count(*) as value
FROM public.chat_messages
...
SELECT 
    'Пользователей с сообщениями' as metric,
    count(DISTINCT u.id) as value
FROM public.users u
INNER JOIN public.chat_messages cm ON u.id = cm.user_id
UNION ALL
SELECT 
    'Пользователей без сообщений' as metric,
    count(*) as value
FROM public.users u
...
UNION ALL
SELECT 
    'Среднее сообщений на пользователя' as metric,
    CASE 
...
-- ПРИНЦИП: Командная работа - все участники проекта видят сообщения проекта
-- СЛЕДУЮЩИЙ ШАГ: widget_development_settings
-- =====================================================
