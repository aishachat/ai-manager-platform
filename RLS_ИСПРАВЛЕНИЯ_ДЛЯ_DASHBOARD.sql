-- 🔒 RLS Исправления для Supabase Dashboard
-- Скопируйте и выполните этот код в SQL Editor

-- ============================================
-- ИСПРАВЛЯЕМ INSERT ПОЛИТИКИ БЕЗ УСЛОВИЙ
-- ============================================
...
-- 9. projects (первая политика)
DROP POLICY IF EXISTS "Users can insert their own projects" ON projects;
CREATE POLICY "Users can insert their own projects" ON projects
    FOR INSERT WITH CHECK (auth.uid() = owner_id);

-- 10. projects (вторая политика)
...
-- 13. users (первая политика)
DROP POLICY IF EXISTS "Users can insert their own data" ON users;
CREATE POLICY "Users can insert their own data" ON users
    FOR INSERT WITH CHECK (auth.uid() = id);

-- 14. users (вторая политика)
DROP POLICY IF EXISTS "Users can insert their own profile" ON users;
CREATE POLICY "Users can insert their own profile" ON users
    FOR INSERT WITH CHECK (auth.uid() = id);

-- 15. users (третья политика)
...
-- Проверяем количество проблемных политик
SELECT 
