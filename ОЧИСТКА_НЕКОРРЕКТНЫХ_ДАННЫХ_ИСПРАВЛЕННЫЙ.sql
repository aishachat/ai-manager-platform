-- =====================================================
-- ОЧИСТКА НЕКОРРЕКТНЫХ ДАННЫХ (ИСПРАВЛЕННЫЙ)
-- =====================================================
...
-- 6. ПРОВЕРЯЕМ СВЯЗИ С ПОЛЬЗОВАТЕЛЯМИ (с приведением типов)
SELECT 
    'messenger_contacts' as table_name,
    count(*) as total_records,
    count(CASE WHEN user_id IN (SELECT id::text FROM public.users) THEN 1 END) as valid_users,
    count(CASE WHEN user_id NOT IN (SELECT id::text FROM public.users) AND user_id IS NOT NULL THEN 1 END) as invalid_users
FROM public.messenger_contacts;
