-- =====================================================
-- ШАГ 15: ИСПРАВЛЕНИЕ RLS ДЛЯ ТАБЛИЦЫ WIDGET_DEVELOPMENT_SETTINGS (ФИНАЛЬНЫЙ)
-- =====================================================
...
-- ПРИМЕЧАНИЕ: user_id должен быть типа uuid (исправлен в предыдущем шаге)
...
-- 9. ПРОВЕРЯЕМ СВЯЗИ С ПОЛЬЗОВАТЕЛЯМИ (без приведения типов)
SELECT 
    'Пользователей с настройками виджета' as metric,
    count(DISTINCT u.id) as value
FROM public.users u
INNER JOIN public.widget_development_settings wds ON u.id = wds.user_id
UNION ALL
SELECT 
    'Пользователей без настроек виджета' as metric,
    count(*) as value
FROM public.users u
LEFT JOIN public.widget_development_settings wds ON u.id = wds.user_id
