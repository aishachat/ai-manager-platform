-- =====================================================
-- ШАГ 16: ИСПРАВЛЕНИЕ RLS ДЛЯ ТАБЛИЦЫ TELEGRAM_SETTINGS (ПРОЕКТ)
-- =====================================================
-- Таблица: настройки Telegram бота для проекта
-- Логика: 1 проект = 1 набор настроек Telegram (командная работа)
...
DROP POLICY IF EXISTS "telegram_settings_owner_access" ON public.telegram_settings;
DROP POLICY IF EXISTS "telegram_settings_user_access" ON public.telegram_settings;
DROP POLICY IF EXISTS "telegram_settings_combined_access" ON public.telegram_settings;
...
SELECT 
    ...
    (SELECT count(*) FROM pg_policies WHERE tablename = 'telegram_settings') as policies_count
FROM pg_tables 
...
-- 7. ПРОВЕРЯЕМ ДАННЫЕ
SELECT 
    'Всего настроек Telegram' as metric,
    count(*) as value
...
SELECT 
    'Проектов с настройками Telegram' as metric,
    count(DISTINCT p.id) as value
FROM public.projects p
INNER JOIN public.telegram_settings ts ON p.id = ts.project_id
UNION ALL
SELECT 
    'Проектов без настроек Telegram' as metric,
    count(*) as value
FROM public.projects p
LEFT JOIN public.telegram_settings ts ON p.id = ts.project_id
WHERE ts.project_id IS NULL;

-- 9. ПРОВЕРЯЕМ СВЯЗИ С ПОЛЬЗОВАТЕЛЯМИ
SELECT 
    'Пользователей с настройками Telegram' as metric,
    count(DISTINCT u.id) as value
FROM public.users u
INNER JOIN public.telegram_settings ts ON u.id = ts.user_id
UNION ALL
SELECT 
    'Пользователей без настроек Telegram' as metric,
    count(*) as value
FROM public.users u
LEFT JOIN public.telegram_settings ts ON u.id = ts.user_id
...
SELECT 
    ...
    'Пользователь должен видеть только настройки Telegram своих проектов' as expected
FROM public.telegram_settings;
...
-- ПРИНЦИП: Командная работа - все участники проекта видят настройки Telegram
