-- 🔧 ФИНАЛЬНЫЕ RLS ПОЛИТИКИ ПО ЛОГИКЕ ПРОЕКТОВ
-- Настройка всех оставшихся таблиц по принципу "1 проект = 1 набор AI инструментов"
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ai_agent_settings') THEN
        ...
        -- Создаем правильную политику (если не существует)
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'ai_agent_settings' AND policyname = 'ai_agent_settings_project_access') THEN
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'bot_corrections') THEN
        ...
        -- Создаем правильную политику (если не существует)
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'bot_corrections' AND policyname = 'bot_corrections_project_access') THEN
